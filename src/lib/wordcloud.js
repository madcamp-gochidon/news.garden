...
if (!globalThis.setImmediate) {
  globalThis.setImmediate = (function setupSetImmediate () {
    return globalThis.msSetImmediate ||
    globalThis.webkitSetImmediate ||
    globalThis.mozSetImmediate ||
    globalThis.oSetImmediate ||
    (function setupSetZeroTimeout () {
      if (!globalThis.postMessage || !globalThis.addEventListener) {
        return null
      }
      ...
      var setZeroTimeout = function setZeroTimeout (callback) {
        ...
        globalThis.postMessage(message + id.toString(36), '*')

        return id
      }

      globalThis.addEventListener('message', function setZeroTimeoutMessage (evt) {
        // Skipping checking event source, retarded IE confused this window
        // object with another in the presence of iframe
        if (typeof evt.data !== 'string' ||
            evt.data.substr(0, message.length) !== message/* ||
            evt.source !== globalThis */) {
          return
        }
      ...
      globalThis.clearImmediate = function clearZeroTimeout (id) {
        if (!callbacks[id]) {
          return
        }
...
if (!globalThis.clearImmediate) {
  globalThis.clearImmediate = (function setupClearImmediate () {
    return globalThis.msClearImmediate ||
    globalThis.webkitClearImmediate ||
    globalThis.mozClearImmediate ||
    globalThis.oClearImmediate ||
...
(function (global) {
  ...
  // Expose the library as an ES6 module or global variable
  if (typeof define === 'function' && define.amd) { // eslint-disable-line no-undef
    globalThis.WordCloud = WordCloud;
...
})(typeof window !== 'undefined' ? window : globalThis); // jshint ignore:line
